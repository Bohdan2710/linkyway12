...
?>
<div class="modal_form_niche">
  <form 
  class="modal_form_niche_wrap"
  action="<?php the_field($form_modal_formspree_url, 1047); ?>"
  method="POST"
  id="form_booking_data"
  >

  <div class="form_niche__close">
    <?php $image = get_field($form_modal_close_button, 1047);
      if( !empty( $image ) ): ?>
        <img src="<?php echo esc_url($image["url"]); ?>" alt="<?php echo esc_attr($image["alt"]); ?>" />
    <?php endif; ?>
  </div>

  <?php if (have_rows($form_modal_inputs, 1047)) : ?>
    <?php while (have_rows($form_modal_inputs, 1047)) : the_row(); 
  ...
  <div class="form_industry__wrap">
    <textarea 
      name="<?php the_sub_field($form_modal_textarea_name, 1047); ?>" 
      placeholder="<?php the_sub_field($form_modal_textarea_placholder, 1047); ?>" 
      class="form-control"
    ></textarea>
  </div>

  <?php if( get_field($form_modal_button_send_text, 1047) ): ?>
    <button class="button_violet" type="submit">
      <?php the_field($form_modal_button_send_text, 1047); ?>
    </button>
  <?php endif; ?>
